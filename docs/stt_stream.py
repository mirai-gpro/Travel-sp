...
1回の発話は2$301C3文以内、10秒以内に収めてください。
...
【インタビュー開始時】
1. 話者を紹介する（「本日は$301C様にお話を伺います」）
2. 自己紹介を依頼する（「まずは自己紹介をお願いします」）
3. 話者の発言を待つ

【通常のインタビュー進行】
発話パターン：相槌 + 次の質問
「$301Cですね。では、$301Cでしょうか？」
...
- 質問は必ず「$301Cでしょうか？」「$301Cですか？」で丁寧に終える
...
class TTSPlayer:
    ...
    def synthesize_and_play(self, text: str, p: pyaudio.PyAudio):
        ...
        # 先に全文を合成（バッファリング）
        audio_buffers = []
        print(f"   $D83D$DD0A TTS合成中... ({len(sentences)}文)")
        for i, sentence in enumerate(sentences):
            ...
            try:
                ...
                print(f"   $26A0$FE0F TTS合成エラー: {e}")
        
        if not audio_buffers:
            return
        
        # 合成完了後、連続再生
        print(f"   $D83D$DD0A TTS再生中...")
        stream = p.open(
            format=FORMAT,
            channels=CHANNELS,
    ...
    def _init_chat(self):
        ...
        if os.path.exists(REFERENCE_PDF_FILE_PATH):
            try:
                ...
                print(f"$D83D$DCC4 参照PDF読み込み完了: {REFERENCE_PDF_FILE_PATH}")
            except Exception as e:
                print(f"$26A0$FE0F PDF読み込みエラー: {e}")
...
"""
    ...
    def _get_next_question_from_script(self) -> str:
        ...
        try:
            ...
            print(f"$26A0$FE0F スクリプト読み込みエラー: {e}")
            return "次の質問に進んでください"
            ...
            async for response in turn:
                ...
                if response.server_content:
                    ...
                    if hasattr(sc, 'turn_complete') and sc.turn_complete:
                        if self.user_transcript_buffer.strip():
                            ...
                            print(f"$D83D$DC64 ユーザー: {user_text}")
                        ...
                        if self.ai_transcript_buffer.strip():
                            ...
                            if is_incomplete:
                                print("   $26A0$FE0F 発言が途中で切れた可能性あり")
                ...
                if response.server_content and hasattr(response.server_content, 'interrupted'):
                    if response.server_content.interrupted:
                        print("$D83D$DEA8 割り込み検知！")
                        if self.ai_transcript_buffer.strip():
                            print(f"$D83E$DD16 AI(中断): {self.ai_transcript_buffer.strip()}")
                            self.ai_transcript_buffer = ""
    ...
    async def _handle_tool_call(self, tool_call, session):
        ...
        for fc in tool_call.function_calls:
            if fc.name == "request_explanation":
                ...
                print(f"$D83D$DD27 説明依頼検知: {topic}")
                
                # ツール結果を返す（説明だけ、次の質問はしない）
                instruction = f"「{topic}」について2$301C3文で簡潔に説明してください。説明が終わったら「以上です」と言って止まってください。次の質問はしないでください。"
                ...
                try:
                    ...
                    print("   $2705 ツール応答送信完了、Live APIの説明を待機中...")
                    # ※ ここでは再接続フラグを立てない！
                    # Live APIが説明を完了した後、通常のターン完了で累積制限により再接続される
                except Exception as e:
                    print(f"$26A0$FE0F ツール応答エラー: {e}")
    ...
    async def run(self):
        ...
        print("\n" + "=" * 60)
        print(f"$D83C$DF99$FE0F  Gemini Live API - {mode_names.get(self.mode, self.mode)}")
        print("    （Live API統一方式 + 自動再接続）")
        print("=" * 60)
        print("$26A0$FE0F  ヘッドセット推奨（エコー防止のため）")
        print("$D83D$DCA1 全てLive APIで処理")
        print(f"$D83D$DCA1 累積{self.MAX_AI_CHARS_BEFORE_RECONNECT}文字で自動再接続")
        print("$D83D$DCA1 Ctrl+C で終了")
        print("=" * 60 + "\n")
        ...
        try:
            while True:
                ...
                if self.session_count > 1:
                    ...
                    print(f"\n$D83D$DD04 セッション再接続 (#{self.session_count})")
                    if context:
                ...
                try:
                    async with self.client.aio.live.connect(
                        ...
                        if self.session_count == 1:
                            print("$2705 Gemini Live API に接続しました。話しかけてください！\n")
                        else:
                            print("$2705 再接続完了。続けてください。\n")
                            ...
                            try:
                                ...
                                print("   $D83D$DCE4 再接続通知を送信しました")
                            except Exception as e:
                                print(f"   $26A0$FE0F 再接続通知送信エラー: {e}")
    ...
    async def _init_audio_streams(self):
        ...
        print(f"$D83C$DFA4 入力: {device_info['name']}")
        ...
        print(f"$D83D$DD0A 出力: {device_info['name']}")
            ...
            while not self.needs_reconnect:
                try:
                    ...
                    try:
                        ...
                        send_count += 1
                        if send_count <= 3:
                            print(f"   $D83C$DFA4 音声送信中... ({send_count})")
                    except asyncio.QueueFull:
                        pass
                except Exception as e:
                    if self.needs_reconnect:
                        return
                    print(f"$26A0$FE0F マイク読み取りエラー: {e}")
                    self.needs_reconnect = True
                    return
        ...
        async def send_audio():
            ...
            while not self.needs_reconnect:
                try:
                    ...
                    # 定期的に送信確認
                    if send_count % 500 == 0:
                        print(f"   $D83D$DCE1 音声送信継続中... ({send_count})")
                except asyncio.TimeoutError:
                    continue
                except Exception as e:
                    if self.needs_reconnect:
                        return
                    print(f"$26A0$FE0F 送信エラー: {e}")
                    self.needs_reconnect = True
                    return
            ...
            try:
                ...
                if any(keyword in error_msg for keyword in ["1011", "1008", "internal error", "closed", "deadline", "policy"]):
                    if "deadline" in error_msg:
                        print(f"$23F1$FE0F サーバータイムアウト。再接続します...")
                    elif "1008" in error_msg or "policy" in error_msg:
                        print(f"$26A0$FE0F ポリシーエラー。再接続します...")
                    else:
                        print(f"$26A0$FE0F 受信エラー（再接続します）: {e}")
                    self.needs_reconnect = True
                else:
                    raise
...
def main():
    ...
    if input_device_index == -1:
        print(f"$274C 入力デバイスが見つかりません: {INPUT_DEVICE_NAME}")
        print("\n利用可能なデバイス:")
        for i in range(p.get_device_count()):
            info = p.get_device_info_by_index(i)
    ...
    if output_device_index == -1:
        print(f"$274C 出力デバイスが見つかりません: {TTS_OUTPUT_DEVICE_NAME}")
        print("\n利用可能なデバイス:")
        for i in range(p.get_device_count()):
            info = p.get_device_info_by_index(i)
    ...
    print(f"$2713 入力デバイス: [{input_device_index}] {INPUT_DEVICE_NAME}")
    print(f"$2713 出力デバイス: [{output_device_index}] {TTS_OUTPUT_DEVICE_NAME}")
