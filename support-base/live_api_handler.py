...
## 会話フロー（1ターン検索を最優先）
- ユーザーが条件を1つでも言ったら、即座に「お探ししますね」と言ってsearch_shopsツールを呼び出す。
- 追加質問は一切しない。予算・人数・シーンなどを聞き返さない。
- 「お探ししますね」の後に質問を続けることは禁止。
- 検索する時は必ずsearch_shopsツールを呼び出すこと。ツールを呼ばずに「探します」と言うだけでは検索は実行されない。
...
### 検索を提案するタイミング
- ユーザーが十分な条件を伝えた時点で、自然に「お探ししますね」と発言し、search_shopsツールを呼び出す
- 全項目が揃うまで待つ必要はない。ユーザーが「もういいから探して」等と言った場合は即座に検索する
- 簡易業態（ラーメン、カフェ等）はエリアだけでも検索可能

### 検索の実行方法（厳守）
検索する時は必ずsearch_shopsツールを呼び出すこと。
ツールを呼ばずに「探します」と言うだけでは検索は実行されない。
search_shopsのuser_requestには、会話で得た条件を自然言語でまとめて渡す。

### 検索提案の例
「銀座の和食、個室があるお店をお探ししますね。」→ search_shops(user_request="銀座 和食 個室 接待")
「渋谷で5000円のイタリアン、探しますね。」→ search_shops(user_request="渋谷 イタリアン 予算5000円")
...
# ============================================================
# ショップ検索ツール定義（LiveAPI function calling）
# LLMが条件が揃ったと判断したらこのツールを呼び出す（REST版のshops配列返却に相当）
# ============================================================

SEARCH_SHOPS_TOOL = {
    "function_declarations": [{
        "name": "search_shops",
        "description": "ユーザーの希望条件でお店を検索する。条件が十分に揃ったと判断したら呼び出す。",
        "parameters": {
            "type": "object",
            "properties": {
                "user_request": {
                    "type": "string",
                    "description": "ユーザーの検索リクエスト全文。エリア、ジャンル、予算、人数などの条件を自然言語でまとめたもの。例: '六本木で接待向けのイタリアン、1人1万円くらい、4名'",
                },
            },
...
"""
    ...
    async def _receive_and_forward(self, session):
        ...
        while not self.needs_reconnect and self.is_running:
            ...
            async for response in turn:
                ...
                # 1. tool_call（search_shops ツール呼び出し）
    ...
    async def _handle_tool_call(self, session, tool_call):
        """
        LLMからのtool_call（function calling）を処理する。
        search_shopsツールが呼ばれた場合、ショップ検索を実行する。
        """
        for fc in tool_call.function_calls:
            logger.info(f"[LiveAPI] tool_call受信: {fc.name}, args={fc.args}")

            if fc.name == "search_shops":
                user_request = fc.args.get("user_request", "")
                logger.info(f"[LiveAPI] search_shops発火: '{user_request}'")

                # tool_responseを返してからセッションを終了し、ショップ検索を実行
                function_response = types.FunctionResponse(
                    id=fc.id,
                    name=fc.name,
                    response={"result": "検索を開始します"},
                )
                ...
                # 現在のバッファを履歴に保存
                if self.ai_transcript_buffer.strip():
                    self._add_to_history("ai", self.ai_transcript_buffer.strip())
                    self.ai_transcript_buffer = ""
                ...
                logger.warning(f"[LiveAPI] 未知のtool_call: {fc.name}")
