...
再接続後も会話履歴が再送されるので確認し、同じ質問を繰り返さないこと。

### 6. このルールの優先順位
1位：本セクション（短期記憶・重複防止） > 2位：質問ルール > 3位：応答スタイル
...
"""
                ...
                try:
                    async with self.client.aio.live.connect(
                        ...
                        if self.session_count == 1:
                            ...
                            logger.info(f"[LiveAPI] 再接続: 履歴再送 + トリガー送信: '{resume_text[:50]}'")
    ...
    async def _send_history_on_reconnect(self, session):
        """
        再接続時に会話履歴をsend_client_content()で再送する（v3.2新規）
